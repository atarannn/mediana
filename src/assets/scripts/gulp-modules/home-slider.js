function init() {
  ...
  const slider = new Swiper('.home-slider', {
    ...
    on: {
      init: (e) => {
        ...
        tlFromBottom(currentSlide.querySelector('.swiper-slide__title')).play();
        tlFromBottom3(currentSlide.querySelector('.swiper-slide__text')).play();
        tlFromBottom3(currentSlide.querySelector('.swiper-slide__link')).play();
      },
    },
  });
  ...
  slider.on('beforeTransitionStart', (obj) => {
    ...
    tlToTop(currentSlide.querySelector('.swiper-slide__title')).play();
    tlToTop3(currentSlide.querySelector('.swiper-slide__text')).play();
    tlToTop3(currentSlide.querySelector('.swiper-slide__link')).play();
  })
...
document.querySelectorAll(".swiper-slide__title").forEach((text) => {
...
function mainScreenGlSlider() {
  ...
  const imagesForGl = Array.from(document.querySelectorAll('.home-slider .swiper-slide:not(.swiper-slide-duplicate) .swiper-slide__right img')).map(img => img.src);
